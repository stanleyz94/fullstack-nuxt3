...
<script setup lang="ts">
    ...
    const registrationRules = {
      ...
      email: { 
        ...
        emailValidator: helpers.withMessage(({ $model }) => `${$model} is not a valid email!`, emailValidator) 
      },
      password: { 
        required: helpers.withMessage('Password is required', required),
        minLength: helpers.withMessage('Password is too short - should be min 6 characters', minLength(6)) 
      },
    }

    const $externalResults = ref({}) 
    const v$ = useVuelidate(registrationRules, registrationForm, { $externalResults })
    ...
    async function postRegisterForm() {
      ...
      // console.log('v$.username.$errors', v$.value.$errors)
      ...
      await registerWithEmail(username.value, name.value, email.value, password.value, fetchKey.value)
      console.log({ externalResults: $externalResults.value, errors: v$.value.$errors, v: v$.value })
      // errors.value = response.val  ue.errors
    };
